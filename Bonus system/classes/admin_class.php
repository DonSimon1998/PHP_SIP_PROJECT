...
class Admin_Class
{
	...
	public function add_new_task($data)
	{
		...
		$bonus = $this->test_form_input_data($data['bonus']);
		...
		try {
			$add_task = $this->db->prepare("INSERT INTO task_info (t_title, t_description, t_bonus, t_start_time, 	t_end_time, t_user_id) VALUES (:x, :y, :d, :z, :a, :b) ");

			$add_task->bindparam(':x', $task_title);
			$add_task->bindparam(':y', $task_description);
			$add_task->bindparam(':d', $bonus);
	...
	public function update_task_info($data, $task_id, $user_role)
	{
		...
		try {
			$update_task = $this->db->prepare("UPDATE task_info SET t_title = :x, t_description = :y, t_bonus = :d, t_start_time = :z, t_end_time = :a, t_user_id = :b, status = :c WHERE task_id = :id ");

			$update_task->bindparam(':x', $task_title);
			$update_task->bindparam(':y', $task_description);
			$update_task->bindparam(':d', $bonus);
